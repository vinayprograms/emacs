...
(use-package evil
  ...
  (defun my-update-mode-line-color ()
    ...
    (progn
      (cond
       ((evil-insert-state-p) (set-face-background 'mode-line "red"))
       ((evil-visual-state-p) (set-face-background 'mode-line "blue"))
       ((evil-replace-state-p) (set-face-background 'mode-line "green"))
       (t (set-face-background 'mode-line "gray20")))
      (cond
       ...
       (t (send-string-to-terminal "\e[1 q")))))  ;; Default color for normal mode
  (defun my-set-default-cursor-before-exit ()
    "Set the cursor to the default block shape before Emacs exits."
    (send-string-to-terminal "\e[2 q")) ;; Block cursor (normal mode)
  (add-hook 'kill-emacs-hook #'my-set-default-cursor-before-exit)

  
  ;; Configure cursor shape for different Evil modes
  (setq evil-normal-state-cursor 'box)        ;; Block cursor for normal mode
  (setq evil-insert-state-cursor 'bar)        ;; Bar cursor for insert mode
  (setq evil-replace-state-cursor 'hbar)      ;; Underscore cursor for replace mode
